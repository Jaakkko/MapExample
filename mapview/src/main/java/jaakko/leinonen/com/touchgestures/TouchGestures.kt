package jaakko.leinonen.com.touchgestures
...
class TouchGestureDetector(context: Context, private val listener: TouchGestureListener) {
    companion object {
        ...
        const val SCROLL_FRICTION = 0.05f
    }
    ...
    private var span: Float = Float.NaN
    ...
    var animationRunning: Boolean = false; private set
    var touchDown: Boolean = false; private set
        ...
        if (event.actionMasked in arrayOf(MotionEvent.ACTION_DOWN, MotionEvent.ACTION_POINTER_DOWN, MotionEvent.ACTION_UP, MotionEvent.ACTION_POINTER_UP, MotionEvent.ACTION_CANCEL)) {
            ...
            // Up && pointercount == 1
                ...
                if (d < 128) {
                    when (animMode) {
                        ...
                        ANIM_MODE_MOVE -> {
                            ...
                            focusY = 0f
                            animationRunning = true
                            mScroller.setFriction(SCROLL_FRICTION)
                ...
                if (tapCount > 0 && eventTime - startTime > 300) {
                    ...
                    val x: Float = event.x
                    val y: Float = event.y
                    if (tapCount == 1 && listener.onClick(this, x, y)) {
                        tapCount = 0
                    }
                }
                if (tapCount == 2) {
    ...
    interface TouchGestureListener {
        ...
        fun onDoubleClick(focusX: Float, focusY: Float)
        /**
         * Return true if handled and onDoubleClick won't be called
         */
        fun onClick(detector: TouchGestureDetector, x: Float, y: Float): Boolean
    ...
    open class SimpleTouchGestureListener : TouchGestureListener {
        override fun onEnd() {}
        override fun onDoubleClick(focusX: Float, focusY: Float) {}
        override fun onClick(detector: TouchGestureDetector, x: Float, y: Float): Boolean = false
